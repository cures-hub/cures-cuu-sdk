class BKMissingInteractionElementFeatureGenerator {
    
    @available(iOS 11.0, *)
    @available(iOS 11.0, *)
    func features(completion: (MissingInteractionElementContextInput?)->Void ) {
        ...
        InteractionKit.shared.fetch(IKViewEventCharacteristics.self, predicate: predicate, completion: { results in
            ...
            if let events = results as? Array<IKViewEventCharacteristics> {
                ...
                if openTasks == 0 {
                    return completion(
                        MissingInteractionElementContextInput(
