class BKUXInconsistencyFeatureGenerator {
    
    @available(iOS 11.0, *)
    func features(completion: (UXInconsistencyContextInput?)->Void ) {
        
        let lock = NSLock()
        var openTasks = 1
        
        let windowSize: TimeInterval = 40
